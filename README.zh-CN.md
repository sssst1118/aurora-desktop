...
- **⚡ 全局搜索** — `Ctrl+Shift+Space` 即呼即搜,开始菜单应用秒级定位,回车直达;支持拼音搜索(`jsq` → 计算器),搜索框直接搜文件
- **🌌 灵动岛** — 常驻屏幕顶端:时间、CPU、内存、网速一眼尽览,置顶不抢焦点
- **📁 文件抽屉** — 桌面文件按类型自动分类收纳,新增即时刷新,桌面从此整洁
- **📋 剪贴板历史** — 自动记录每一次复制,可搜索、一键回贴,重启不丢
- **🧲 Dock 栏** — 把任意 exe / 快捷方式拖进搜索框底部图标排:一键启动、运行绿点、拖拽排序
- **🤖 AI 助手** — DeepSeek 云端 + Ollama 本地双模式;自然语言打开应用、搜索文件、更换壁纸
- **🖼️ 动态壁纸** — 视频 / HTML(网页素材内可跑 WebGL)注入桌面底层,笔记本自动降载,多屏热插拔自动重建,可逐屏配置
- **📊 系统监控** — CPU / 内存 / 网络实时采样,灵动岛与托盘双处可见
- **🎨 主题系统** — 浅色 / 深色 / 跟随系统 + 4 色强调色,全部窗口即时切换
